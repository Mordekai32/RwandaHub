    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100">
      ...
      <div className="relative overflow-hidden pt-12 pb-16 px-4">
        ...
        <div className="relative max-w-7xl mx-auto text-center">
          <div className="inline-flex items-center gap-2 bg-white/80 backdrop-blur-sm rounded-full px-4 py-2 shadow-sm mb-6 border border-slate-200/50">
            <FaHeadset className="text-blue-500" />
            <span className="text-sm text-slate-600">We're Here to Help</span>
          </div>
          <h1 className="text-4xl md:text-5xl font-bold bg-gradient-to-r from-slate-800 via-blue-600 to-emerald-500 bg-clip-text text-transparent mb-4">
            Support Center
          </h1>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-12">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {supportOptions.map((option, idx) => (
            <div key={idx} className="bg-white/80 backdrop-blur-sm rounded-xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden border border-slate-200/50">
              ...
              <div className="p-6 text-center">
                ...
                <h3 className="text-xl font-bold text-slate-800 mb-1">{option.title}</h3>
                <p className="text-slate-500 text-sm mb-4">{option.detail}</p>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-20">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {/* FAQ Section */}
          <div className="bg-white/80 backdrop-blur-sm rounded-xl shadow-md p-6 border border-slate-200/50">
            <div className="flex items-center gap-2 mb-4">
              <FaComments className="text-blue-500 text-xl" />
              <h2 className="text-2xl font-bold text-slate-800">Frequently Asked</h2>
            </div>
            <div className="space-y-4">
              {faqs.map((faq, idx) => (
                <details key={idx} className="group border-b border-slate-100 pb-3">
                  ...
                  <p className="text-slate-500 text-sm mt-2 pl-2">{faq.answer}</p>
                </details>
              ))}
            </div>
            <div className="mt-6 text-center">
              <a href="/help-center" className="text-blue-600 hover:text-blue-700 text-sm font-medium">View all FAQs →</a>
            </div>
          </div>
            ...
            {submitted ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  <input type="text" name="name" required value={formData.name} onChange={handleChange} className="w-full border border-slate-200 rounded-lg px-4 py-2 bg-slate-50 focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 transition" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Email Address</label>
                  <input type="email" name="email" required value={formData.email} onChange={handleChange} className="w-full border border-slate-200 rounded-lg px-4 py-2 bg-slate-50 focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 transition" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Message</label>
                  <textarea name="message" rows="4" required value={formData.message} onChange={handleChange} className="w-full border border-slate-200 rounded-lg px-4 py-2 bg-slate-50 focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 transition" />
                </div>
        ...
        {/* Support Hours */}
        <div className="mt-8 bg-white/50 backdrop-blur-sm rounded-lg p-4 text-center border border-slate-200/50">
          <FaClock className="inline text-blue-500 mr-2" />
          <span className="text-slate-600">Support hours: Monday–Friday, 8am–5pm (CAT). Responses within 24 hours.</span>
        </div>
      </div>
    </div>
  );
