...
// ========== NEW: Logo upload by company ID (for frontend) ==========
app.put('/api/companies/:id/logo', verifyToken, uploadLogo.single('logo'), async (req, res) => {
  try {
    const company = await Company.findById(req.params.id);
    if (!company) return res.status(404).json({ message: 'Company not found' });
    
    // Allow admin or the company owner to update logo
    if (req.user.role !== 'admin' && company.ownerId.toString() !== req.user.id) {
